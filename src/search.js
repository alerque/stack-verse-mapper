...
// Search one or more indexes: indexes must be an object of individual indexes
module.exports.search = function( query, indexes, options )
{
	...
	_.forIn( indexes, function( index, sitename )
	{
		var set_hits = {};
		var site_results = index.posts.map( function( post )
		{
		...
		site_results.forEach( function( post )
		{
		...
		results = results.concat( site_results );
	});
